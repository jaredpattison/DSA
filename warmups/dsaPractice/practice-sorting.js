...
const quickSort = (items, compare, left, right) => {
  const _partition = (items, compare, left, right, pivot) => {
    pivot = pivot || Math.floor((left + right) /2);

    if (left <= right) {
      left = _advanceLeft(items, compare, left, pivot);
      right = _advanceRight(items, compare, left, pivot);
      if (left <= right) {
        if (left < right) _swap(items, left, right);
  ...
  const _swap = (items, left, right) => {
    let tmp = items[left];
    items[left] = items[right];
    items[right] = tmp;
  };

  const _advanceLeft = (items, compare, left, pivot) => {
    if (compare(items[left], items[pivot]))
      return _advanceLeft(items, compare, left + 1, pivot);
    return left;
  };

  const _advanceRight = (items, compare, right, pivot) => {
