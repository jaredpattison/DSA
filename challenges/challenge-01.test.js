...
const removeElements = (arr, callback) => {
  for (let i of arr) callback(i, arr);
  return arr;
}
...
const fizzbuzz = (arr) => {
  const res = [];
  arr.forEach( num => {
    if (!(num % 5) && !(num % 3)) return res.push('Fizz Buzz');
    if (!(num % 5)) return res.push('Buzz');
    if (!(num % 3)) return res.push('Fizz');
    return res.push(num);
  });
  return res;
}
